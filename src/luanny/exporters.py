...
def export_results(
    ...
    """
    export_dir = base_output_dir if base_output_dir else Path(config.output_dir)
    export_dir.mkdir(parents=True, exist_ok=True)
    
    logger.info("exporting_data", total_posts=len(result.posts), export_dir=str(export_dir))
...
def _export_csv(result: CollectionResult, export_dir: Path) -> Path:
    ...
    for p in result.posts:
        ...
        row = {
            ...
            "caption_preview": (p.caption or "").replace("\n", " ").replace("\r", " ")[:150],
            "alts_joined": " | ".join(alts) if alts else "",
            "has_auto_alt_tag": any(m.is_auto_generated_alt for m in p.media),
            "has_manual_alt_tag": any(m.alt_text_source == AltTextSource.MANUAL for m in p.media),
        }
        rows.append(row)
        
    df = pd.DataFrame(rows)
    # Codificação utf-8-sig embute o BOM para compatibilidade nativa com Excel
...
def _export_report(result: CollectionResult, export_dir: Path) -> Path:
    ...
    images_found = sum(1 for p in result.posts if not p.is_carousel and not p.is_video)
...
## Métricas
- **Posts Coletados (Sucesso Total/Parcial):** {success_posts} / {meta.max_posts_requested} solicitados
...
- Auto-Generated Alts identificados pelo Robô: {sum(any(m.is_auto_generated_alt for m in p.media) for p in result.posts)}
- Manual Alts identificados pelo Robô: {sum(any(m.alt_text_source == AltTextSource.MANUAL for m in p.media) for p in result.posts)}

---
*Gerado por Luanny Scraper v{meta.collector_version}. Valide os JSON e CSV em anexo.*
"""
